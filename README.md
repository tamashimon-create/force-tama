...
[GitHub Actions]
   ...
   ├─> sf-propagate.yml      : main/staging への PR マージ → 下位ブランチへ変更を伝播
...
- 順序が守られていない場合は PR の Annotations に**赤いエラーバナー**を表示する
...
> **注意:** `check-promotion-order` は現在**ブロックなし・警告のみ**の動作のため、Required Status Check には追加しない。PR の Annotations にエラーバナーが表示された場合はレビュアーが確認して判断する。
